...
@Aspect    //该标签把ControllerLog类声明为一个切面
...
public class ControllerLog {
     ...
     */
    @Pointcut("execution(public * com.ssm.wxj.controller.*Controller.*(..))")
    ...
    public void beforMethod(JoinPoint joinPoint) throws JsonProcessingException {
        String fullClassName = joinPoint.getTarget().getClass().getName();
        String methodName = joinPoint.getSignature().getName();
        List<Object> args = Arrays.asList(joinPoint.getArgs());
        logger.info("{}.{}.begin.param:{}",fullClassName,methodName,mapper.writeValueAsString(args));
    }
    /**
    ...
    public void afterReturnMethod(JoinPoint joinPoint,Object result) throws JsonProcessingException {
        String fullClassName = joinPoint.getTarget().getClass().getName();
        String methodName = joinPoint.getSignature().getName();
        logger.info("{}.{}.return.result:{}",fullClassName,methodName,mapper.writeValueAsString(result));
    }
    /**
     * 异常通知（方法发生异常执行的代码）
